...
type ExercisePayload = { title: string; instructions: string; starterCode: string; solutionCode: string; expectedOutput: string; hints: string[]; files?: ExerciseFile[]; steps?: string[]; acceptanceCriteria?: string[] };
...
const CodeBlock = ({ code }: { code: string }) => {
  ...
  return (
    <div className="relative group">
      <pre className="max-h-72 overflow-auto rounded-lg bg-black/40 p-4 text-xs text-white/80 font-mono leading-relaxed">{code}</pre>
...
const AdminExercisePage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/exercises/generate-structured`, {
        method: "POST",
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}
            ...
            <div className="space-y-4">
              {[
                ...
                { label: "Tipo de ejercicio", value: exerciseType, onChange: setExerciseType, options: [{ value: "completar_codigo", label: "Completar código" }, { value: "corregir_errores", label: "Corregir errores" }] },
                ...
                <motion.div key="result" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                  <div className="bg-[#242424] border border-white/10 rounded-2xl p-6">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      ...
                      <button type="button" onClick={handleDownloadNotebook}
                        ...
                        <Download className="w-4 h-4" /> Descargar .ipynb
                      </button>
                    </div>
                    <p className="mt-4 text-white/70 text-sm leading-relaxed">{result.exercise.instructions}</p>
                    {result.exercise.files && result.exercise.files.length > 0 && (
                      <div className="mt-4 flex flex-wrap gap-2">
                        {result.exercise.files.map((f) => (
                          <div key={f.filename} className="flex items-center gap-1.5 text-xs rounded-lg border border-white/10 bg-white/5 px-3 py-1.5">
                      ...
                      <ol className="space-y-2">
                        {result.exercise.steps.map((step, i) => (
                          <li key={i} className="flex items-start gap-3 text-sm text-white/65">
                            <span className="mt-0.5 w-5 h-5 rounded-full bg-[#FF5722]/20 text-[#FF5722] text-xs flex items-center justify-center shrink-0 font-medium">{i + 1}</span>
                            {step}
                          </li>
                        ))}
                      </ol>
                          ...
                          <ul className="space-y-1.5">
                            {result.exercise.hints.map((hint, i) => (
                              <li key={i} className="flex items-start gap-2 text-xs text-white/55">
                                <span className="text-[#FF5722] mt-0.5">›</span>{hint}
                              </li>
                            ))}
                          </ul>
                        </>
                        ...
                        <ul className="mt-3 space-y-2">
                          {result.exercise.acceptanceCriteria.map((c, i) => (
                            <li key={i} className="flex items-start gap-2 text-sm text-white/60">
                              <CheckCircle2 className="w-4 h-4 text-green-400 mt-0.5 shrink-0" />{c}
                            </li>
                          ))}
                        </ul>
                      )}
